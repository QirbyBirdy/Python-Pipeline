...
- `Scripts/problem_inventory.py` — a written, counted list of data-quality
  issues in five variables of interest (sex, age, hours worked, ISIC
  industry, ISCO occupation), cross-referenced against what `clean.py`
  actually does about each one
- `Scripts/clean.py` — the actual cleaning stage: exact-duplicate removal,
  two verified auto-corrections (age `-1` and hours `99` sentinels →
  `NaN`), flag-only validation rules (arithmetic consistency, skip-pattern
...
- `Outputs/` — regenerated by running `ingest.py`, `diagnose.py`,
  `clean.py`, `problem_inventory.py`, then `visualize.py`, in that order
...
Scripts/
  ...
  problem_inventory.py        written, counted data-quality findings for sex/age/hours/ISIC/ISCO
  clean.py                   dedup removal + validation rules (flag/correct) + auto-consolidation + ISIC/ISCO classification + parquet/xlsx export
...
Brief Step 5: a written, counted list of data-quality issues for five
variables of interest — sex, age, hours worked, industry (ISIC), occupation
(ISCO) — read-only on the *typed* (pre-cleaning) `ind` data, so it documents
what's wrong before describing what fixes it. Writes
`Outputs/problem_inventory/ind_problem_inventory.md`. Every row of the
table's "Addressed by" column points at the exact `clean.py` function (or
says "not addressed" / "N/A — correctly not imputed" when nothing should
touch it).
...
| Hours worked | 88 rows, `99` sentinel in `q3_03` | the most issues of the five, but one root cause — correcting it also resolves the `q3_05` arithmetic-mismatch flag |
...
Four things happen automatically to the data: exact-duplicate removal (0
found), the age `-1` → `NaN` correction (12 rows `ind` / 28 `emig`), the
hours `99` → `NaN` correction (88 rows in `q3_03`, 6 in `q3_05`), and
free-text consolidation at ≥0.98 similarity (436 variants merged across 6
`ind` columns, 4 in `emig` — see below). Everything else — arithmetic
mismatches, the employment skip-logic check, monetary outliers — is
flag-only, written to `flagged_for_review.csv`, never altered in the
cleaned data. This matches the brief's "flag or correct, depending on
what's defensible" — only verified, evidence-backed patterns get corrected
automatically; everything with real judgment involved gets flagged instead.
...
- **Hours sentinel**: `q3_03` (usual hours, main job) had **88** values of
  exactly `99`, with every other value capping at 98 — a classic "99 or
  more" top-code. `q3_05` had 6 more, matching the earlier
  usual-hours-mismatch flags exactly. Checked every other hours-related
  column (`q3_04`, `q3_08`, `q3_09`, `q3_10`, `q3_07_1..7`) before
  correcting anything: **zero** of them ever use `99`, despite some going
  as high as 168 — confirms this is a sentinel specific to these two
  "usual hours" fields, not a general hours convention. Both corrected to
  `NaN`. The usual-hours-mismatch flag count dropped from 6 to 0 as a
  direct consequence — it was never an independent arithmetic error, just
  a symptom of the uncorrected sentinel.
...
- **Monetary outliers**: 584 total flags across 19 fields (1.5×IQR), led by
