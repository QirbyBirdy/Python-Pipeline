...
- `Scripts/visualize.py` — renders those three distributions as JPEG donut
  charts
- `Outputs/` — regenerated by running `ingest.py`, `diagnose.py`, then
  `visualize.py`

No cleaning (value transformation) or automated report generation exists
yet — dtype conversion is "type coercion" (explicitly in scope per the
brief, Section 3.1), and diagnostics/charts are read-only by design, none
of that is a cleaning step.
