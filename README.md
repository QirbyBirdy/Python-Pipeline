...
- `Scripts/visualize.py` — renders those three distributions as JPEG bar
  charts
...
Scripts/
  ...
  visualize.py                renders the named-variable distributions as JPEG bar charts
Outputs/
...
status) as horizontal bar charts, saved as JPEG to `Outputs/charts/`:
`sex_distribution.jpg`, `age_group_distribution.jpg`,
`employment_status_distribution.jpg`. One consistent theme across all three
(same palette, single blue hue since each is a single-series chart — the
axis labels already carry category identity, so per-bar color would be
decorative, not informative), horizontal bars so the longer category text
(employment status) doesn't collide, and a direct value label at every bar's
tip since each chart has at most 7 bars. `Scripts/visualize.py` reuses
`diagnose.py`'s `value_counts_labeled()`/`age_group_distribution()` rather
than recomputing anything, so the chart numbers always match the diagnostics
report.
